...
function MakeTracks(library){
    ...
    $.each(library, function(index, item){
        ...
        $new = {'code':r, 'id':item.id, 'name':item.name, 'file':item.file, "gain":item.gain, "pan":item.pan, "icon":item.icon, "loop":item.loop, "object":null };
        mytracks.push($new);
        ...
        var url = 'audio/'+item.file;
        var $index = mytracks.findIndex(x => x.code == r);
        mytracks[$index].object = new Audio(url);
    
        // when loaded:
        $(".dot[target="+r+"]").removeClass("loading");
        FadeInAudioNew(r, item.gain);
...
function FadeOutAudio (target) 
    ...
    var fadeAudio = setInterval(function () 
        {
        if ( (sound.volume - 0.01) < 0 ) { sound.volume = 0 }
        if (sound.volume <= 0) { clearInterval(fadeAudio); sound.volume = 0; sound.pause(); DeleteTrack(target); }
        sound.volume -= 0.01;
        }, 300);
    }
...
function FadeInAudio (target, gain) 
    ...
    function FadeInAudioNew (target, gain) 
    {

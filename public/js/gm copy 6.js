...
function MakeTracks(library){
    ...
    $.each(library, function(index, item){
        ...
        $new = {'code':r, 'id':item.id, 'name':item.name, 'file':item.file, "gain":item.gain, "pan":item.pan, "icon":item.icon, "loop":item.loop, "audio":sound };
        mytracks.push($new);
        ...
        var x = GetX(item.pan);
        var y = GetY(item.gain);

        var $element = "<div target='"+r+"' id='"+item.id+"' file='"+item.file+"' gain='"+item.gain+"' loop='"+item.loop+"' style='left:"+x+"px; top:"+y+"px' class='draggable dot loading noselect'><div class='sound-button play-button on'></div><div class='trackname'>"+item.name+"</div><div class='sound-button loop-button'></div></div>";
        $("#arena").append($element);
        MakeDraggable();

        $index = mytracks.findIndex(x => x.code === r);
...
function GeneratePixiSound(code, file, gain){
    console.log("Adding pixi sound: "+code+" with file "+file);
        PIXI.sound.add(r, {
        url: 'audio/'+file,
        preload: true,
        loaded: function() {
        ...
        //FadeInAudio(id, gain);
        //MakeDraggable();
         $(".dot[target="+code+"]").removeClass("loading");
         FadeInAudioNew(code, gain);
        }
    });
}
...
$.each(array, function(index, item){
    ...
    if ( Existing(item.file)) 
        ...
        GenerateTrackNew(item.id, item.name, item.file, item.gain, item.pan, item.icon, item.loop);
...
function GenerateTrackNew(id, name, file, gain, pan, icon, loop){
    
        console.log("Adding "+id+" to the canvas");
        $new = {'id':id, 'name':name, 'file':file, "gain":gain, "pan":pan, "icon":icon, "loop":loop, "object":null };
        mytracks.push($new);
        console.log(mytracks);
    
        // update current preset if new
        ...
        mytracks[$index].object = new Audio(url);
...
function ReGenerateDot(id, name, file, gain, pan, icon, loop) {
    console.log("Re-Adding "+id+" to the canvas with name "+name);
    //console.log(gain);
...
function AddSound(id, name, file, gain, pan, loop){
    ...
    $(".dot[target="+id+"]").removeClass("loading");
    //FadeInAudio(id, gain);
    mytracks[$index].object.play();
    console.log(mytracks[$index].object);
...
function ReAddSound(id, name, file, gain, pan, loop){
    ...
    console.log("Re-Adding Sound");

    //SeedSound(id, name, file, gain, pan, loop);
    var $index = mytracks.findIndex(x => x.id === id);
    var sound = mytracks[$index].object;
...
function FadeOutAudio (target) 
    ...
    var sound = mytracks[$index].object;
    sound.volume = sound.volume.toFixed(2);
    console.log(sound.volume);

    var ninth = (sound.volume / 45).toFixed(2);
    ...
    var fadeAudio = setInterval(function () 
        {
        if ( mytracks[$index].status == 1 ) { console.log("Interrupted"); clearInterval(fadeAudio); }
        sound.volume = (sound.volume - ninth).toFixed(2);
        if (sound.volume <= 0) { clearInterval(fadeAudio); sound.volume = 0; sound.pause(); }
        }, 200);
    }
    ...
    function FadeOutAudioNew (target) 
        ...
        $(".dot[target="+target+"]").fadeOut(600, function() { $(this).remove(); });
        ...
        setTimeout(function(){ sound.unload(); DeleteTrack(target); }, 3000);
...
function FadeInAudio (target, gain) 
    ...
    var fadeAudio = setInterval(function () 
        {
        if ( mytracks[$index].status == 0 ) { console.log("Interrupted"); clearInterval(fadeAudio); }
        sound.volume += 0.01;
        console.log(sound.volume);
...
socket.on("changevolume", function(data){ 
    ...
    var $index = mytracks.findIndex(x => x.id === data.id);
    var it = mytracks[$index].object;
...
socket.on("changepan", function(data){
    var $index = mytracks.findIndex(x => x.code === data.code);
    var it = mytracks[$index].object;

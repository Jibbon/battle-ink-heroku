...
function GenerateTrackNew(id, name, file, gain, pan, icon, loop){
        ...
        $new = {'code':time, 'id':id, 'name':name, 'file':file, "gain":gain, "pan":pan, "icon":icon, "loop":loop, "object":null };
        mytracks.push($new);
        ...
        var $element = "<div code='"+time+"' target='"+id+"' file='"+file+"' gain='"+gain+"' loop='"+loop+"' style='left:"+x+"px; top:"+y+"px' class='draggable dot loading noselect'><div class='sound-button play-button on'></div><div class='trackname'>"+name+"</div><div class='sound-button loop-button'></div></div>";
...
function FadeOutAudio (target) 
    ...
    var fadeAudio = setInterval(function () 
        ...
        else if (sound.volume <= 0) { clearInterval(fadeAudio); DestroySound(target); }

        sound.volume = (sound.volume - 0.01 ).toFixed(2);
        console.log(sound.volume);
        }, 200);
    }
...
$(document).on("click", ".loop-button", function(e){
    e.preventDefault;
    $(this).toggleClass("on");
    var sound = $(this).parent().attr("id");
    var code = $(this).parent().attr("code");

    console.log(sound);
    if ( $(this).hasClass("on") )
        {
        Loop(sound, code, true) 
        }
    else 
        {
        Loop(sound, code, false)
        }
        return false;
});
...
function Loop(id, code, toggle){
    var $data = {"room":$room, "id": id, "code":code, "loop":toggle };
...
$(document).on("click", ".sound-item", function(){
    ...
    if ( $(this).hasClass("selected") ) { GenerateTrackNew(item.id, item.name, item.file, item.gain, item.pan, item.icon, item.loop); }
    else { KillSound(id); }
